...
float3 bandColor( float3 pos , float3 cCol ,  float3 bandCol, float bandSize ,float band ){
	...
	return lerp( cCol , bandCol , clamp( (bandSize-(.2*noise(pos*10))-band) / bandSize,0,1));
            ...
            float d = length( v.worldPos - _WorldSpaceCameraPos);
                    // Triplanar mapping
            ...
            float3 col = hsv( sin(length(color.xyz)*10 + _Time.y) ,.4 ,  abs(sin(length(color.xyz)*100 + _Time.y)));

            col *= abs(sin(length(color.xyz*10 + _Time.y * 2)));
            d += .1*noise( v.worldPos * 10);
            //col *= sin(20*d+ noise(v.worldPos*100)-_Time.y);//float3(1,1,1) * .1 / d;
           // col *= 1*pow(length(col),4);
             col = length(col) *length(col) * length(col)* texCUBE( _CubeMap , v.nor );///v.nor * .5 + .5;//.1 * clamp(col, float3(0,0,0),float3(1,1,1)) / d;
           	col.r = clamp(sin( d * 10 + _Time.y),0,1);
           	col.g = clamp(sin(  d * 10.2 + _Time.y),0,1);
           	col.b = clamp(sin(  d * 10.4 + _Time.y),0,1);
