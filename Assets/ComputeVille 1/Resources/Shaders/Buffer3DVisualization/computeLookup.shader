...
RWStructuredBuffer<Vol> _volumeBuffer;
  ...
  if( abs(pos.x) < 1 &&abs(pos.y) < 1 &&abs(pos.z) < 1){
    ...
    return -1;
      ...
      varyings vert (appdata v){
        ...
        float id = GetID(mPos);
        if( id >= 0 ){
      		Vol vol = _volumeBuffer[int(id)];
      		o.debug = (vol.nor* .5 +.5);///vol.dist;//float3(sin(vol.dist*20),0,0);
      ...
      float4 frag (varyings v) : COLOR {
      		...
      		//tex3D(_MainTex, pos  + .5 );

      		float3 col = v.debug;//* .5 + .5;//float3(1,1,1);//normalize(v.debug) * .5 + .5;
