...
float3 bandColor( float3 pos , float3 cCol ,  float3 bandCol, float bandSize ,float band ){
	...
	return lerp( cCol , bandCol , clamp( (bandSize-(.2*noise(pos*10))-band) / bandSize,0,1));
            ...
            float n = noise( v.worldPos * (10+30 * _Slider2) + float3(0,1,0) * _Time.y );

            if( n  < _Slider1 ){ discard; }
            col = hsv( n * _Slider3*2, 1,1);//hsv( n+_Time.y*.4,.5,n) + hsv(v.closest+_Time.y*.3+v.cID*.2+noise(v.worldPos*10),.7,1)*saturate(.01*sin( v.closest*100+3*noise(v.worldPos*10)) / ( v.closest*v.closest*v.closest));
